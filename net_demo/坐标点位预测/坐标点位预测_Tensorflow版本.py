...
b2 = tf.Variable(tf.constant(0.01, shape=[output_size]))
...
plt.scatter(x1, x2, color=np.squeeze(x_y_color))  # squeeze去掉纬度是1的纬度,相当于去掉[['red'],[''blue]],内层括号变为['red','blue']
# 把坐标xx yy和对应的值probs放入contour函数，给probs值为0.5的所有点上色  plt.show()后 显示的是红蓝点的分界线
plt.contour(xx, yy, probs, levels=[.9])
plt.show()
